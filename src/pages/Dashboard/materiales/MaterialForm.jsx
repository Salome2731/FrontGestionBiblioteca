...
const MaterialForm = ({ isOpen, onClose, onSave, materialAEditar }) => {
    ...
    const handleSubmit = (e) => {
        ...
        // Validación: Cantidad disponible no puede ser mayor a la total
            ...
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-lg shadow-2xl">
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <input 
                        ...
                        required 
                        value={formData.cantTotal} 
                        onChange={(e) => setFormData({...formData, cantTotal: e.target.value})}
                    />
                    <input 
                        type="number" 
                        min="0" 
                        className="border p-2 rounded" 
                        placeholder="Cant. Disp." 
                        required 
                        value={formData.cantDisp} 
                        onChange={(e) => setFormData({...formData, cantDisp: e.target.value})}
                    />
