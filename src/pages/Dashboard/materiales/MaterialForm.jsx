...
const MaterialForm = ({ isOpen, onClose, onSave, materialAEditar }) => {
    const [formData, setFormData] = useState({
        ...
        isbn: '', cantTotal: '', cantDisp: '', estado: 'Disponible'
    });
    ...
    useEffect(() => {
        if (materialAEditar) {
            ...
            setFormData({ titulo: '', autor: '', categoria: '', editorial: '', isbn: '', cantTotal: '', cantDisp: '', estado: 'Disponible' });
        }
    }, [materialAEditar]);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-lg shadow-2xl">
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <input type="number" className="border p-2 rounded" placeholder="Cant. Total" required value={formData.cantTotal} onChange={(e) => setFormData({...formData, cantTotal: e.target.value})}/>
                    <input type="number" className="border p-2 rounded" placeholder="Cant. Disp." required value={formData.cantDisp} onChange={(e) => setFormData({...formData, cantDisp: e.target.value})}/>
